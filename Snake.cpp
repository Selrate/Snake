	...
	while (Restart)
	{
		while (!Lost)
		{
			...
			if (MoveTimer >= 100000)
			{
				...
				// Lose if you go offscreen
				if (
					HeadCoord.X < 0 || HeadCoord.X > MainUtils->GetConsoleCharLenthX() ||
					HeadCoord.Y < 0 || HeadCoord.Y > MainUtils->GetConsoleCharLenthY())
				{
					Lost = true;
				}
		...
		system("cls");
		cout << "Game Over!" << endl;
		cout << "You ate " << SnakeLength << " pickups!" << endl;
		cout << "Press 'r' to restart or 'q' to quit";

		// Get ending input
		bool InputCycle = true;
		while (InputCycle)
		{
			switch (_getch())
			{
